...
ga4_data as 
    ...
    sum(sessions) as sessions, sum(engaged_sessions) as engaged_sessions, sum(conversions_purchase) as ga4_purchases, sum(purchase_value) as ga4_revenue
    FROM {{ source('ga4_raw','traffic_sources') }}
    GROUP BY 1,2,3
    UNION ALL
